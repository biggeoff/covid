...
def findCases(arctic_dir, worklist):
    """
    find all of the 
    """
    fullpaths = glob(arctic_dir+'/qc_pass_climb_upload/'+worklist+'/*/')
...
if __name__ == "__main__":
    ...
    run = args.arctic[0].strip('/').split('/')[-2]

    log = open(args.arctic[0]+'/ocarina.log', 'a')
    err = open(args.arctic[0]+'/ocarina.err', 'a')
    cases = findCases(args.arctic[0], args.worklist[0])
    for case in cases:
